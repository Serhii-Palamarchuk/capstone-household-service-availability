...
Для вкладених `Service` деталізація має доходити до кінцевих причин типу `Device` або `ExternalProvider`. Точний алгоритм і формат причинних шляхів фіксуються в `docs/SIMULATION.md` до реалізації simulation engine.
