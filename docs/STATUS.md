...
Оновлено: 2026-09-03 (після A4-верстки, фінальної пагінації, PDF visual QA та закриття останнього supervisor comment).
...
- Результат Week 6 — готова пояснювальна записка, не чернетка.
...
Останній відкритий comment `додати що буде вміти проєкт` Сергій закрив 2026-09-03. Повторна перевірка Google Docs comments API підтвердила `resolved = true`; відкритих supervisor comments у робочій записці більше немає.

## Постійні правила редагування записки

- Усе, що ChatGPT/Codex додає або змістовно змінює, позначати помаранчевим `#E65900`; незмінений текст залишати чорним.
- Важливо: у поточному Google Doc значна частина раніше змінених фрагментів зараз відображається чорним, тобто попереднє orange-маркування частково втрачено під час formatting/layout pass. Не виконувати масове перефарбування автоматично; якщо відновлювати — лише точково за журналом змін.
...
1. Сергій переглядає фінальну A4-версію.
2. Визначити, чи потрібно відновлювати частково втрачене orange-маркування перед фінальним затвердженням; якщо так — робити лише точково, не змінюючи Justify/верстку.
3. Після фінального підтвердження зробити final PDF і передати Тетяні готову записку.
4. Після завершення записки підготувати й відрепетирувати 15-хвилинне фінальне демо та Week 6 materials.
